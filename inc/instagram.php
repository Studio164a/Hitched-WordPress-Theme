...
class OSF_Instagram {
	...
	public function __construct() {
		$this->token = hitched_get_option('instagram_token');
    ...
    public function shortcode($atts = array()) {
        ...
        if ( $this->token === false )
            return __( 'Unable to fetch Instagram photos. No access token set in theme settings.', 'hitched' );
        ...
        if ( strlen($args['username']) < 2 )
            return __( 'Invalid Instagram username.', 'hitched' );
    ...
    public function get_user_id($username) {
    	if ( $this->token === false ) {
            $this->set_error( __( 'No Instagram access token set.', 'hitched' ) );
    		return false;
        }
        ...
        if ( $user === false ) {
        	...
        	$url = sprintf( "https://api.instagram.com/v1/users/search?q=%s&access_token=%s", urlencode( $username ), $this->token );

	        $response = wp_remote_get( $url );

	        if ( is_wp_error( $response ) ) {
                $this->set_error( __( 'Invalid Instagram username.', 'hitched' ) );
                return false;
            }
    ...
    protected function get_photos($user_id, $args = array()) {
    	...
    	// Check cache
    	$transient_key = sprintf( "instagra_%d_%d", $user_id, $count );
		...
		if ( $images === false ) {
            ...
            if ( is_wp_error( $response ) ) { 
                return _e( 'Unable to load images from Instagram', 'hitched' );
            }
