...
class OSFA_Twitter_Widget extends WP_Widget {
    ...
    function OSFA_Twitter_Widget() {
        ...
        $this->WP_Widget('OSFA_Twitter_Widget', __('Twitter by Studio 164a', 'hitched'), $widget_ops);
    }
    ...
    function form($instance) {
        ...
        <?php if ( !$this->has_authorized() ) : ?>
            ...
            <p>
                <strong><?php printf( __( 'Before using the Twitter widget, you need to set up Twitter integration on the %stheme settings page%s.', 'hitched' ), '<a href="'.admin_url('themes.php?page=hitched-options').'">', '</a>' ) ?></strong>
            </p>
    ...
    function widget($args, $instance) {
        ...
        if ( $this->has_authorized() === false ) {
            echo '<p>' . __( 'Missing Twitter app credentials.', 'hitched' ) . '</p>';            
        }
        else {                    
            ...
            if ($tweets === false) {
                ...
                $twitter = new TwitterOAuth(hitched_get_option('twitter_consumer_key'), hitched_get_option('twitter_consumer_secret'), hitched_get_option('twitter_token'), hitched_get_option('twitter_token_secret'));
            ...
            <ul>
                ...
                <li class="floatright"><?php printf( '<a href="%s" title="%s"><i class="icon-twitter"></i> %s</a>', "http://twitter.com/$name", _x( 'Click to go to our Twitter profile', 'twitter profile link title attribute', 'hitched' ), __( 'Follow us on Twitter', 'hitched' ) ) ?></li>
            </ul>        
    ...
    protected function has_authorized() {
        return hitched_get_option('twitter_token') 
            && hitched_get_option('twitter_token_secret') 
            && hitched_get_option('twitter_consumer_secret') 
            && hitched_get_option('twitter_consumer_key');
    }
}